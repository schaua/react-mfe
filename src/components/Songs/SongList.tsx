...
const SongList = ({ initialSongs }: { initialSongs: SongType[] }): JSX.Element => {
...
//   ];
  ...
  const addVote = (id: number): void => {
    const newSongs = songs.map((song, idx) =>
      idx === id ? { ...song, votes: song.votes + 1 } : song
    );
    setSongs(newSongs);
  };
  ...
  return (
    <div>
      ...
      <ul style={{ display: visible ? "block" : "none" }}>
        ...
        {showAll &&
          songs.map((song, index) => (
            <Song key={index} song={song} recordVote={() => addVote(index)} />
          ))}

        {!showAll &&
          songs
            .filter((it) => it.votes >= 2)
            .map((song, index) => (
              <Song key={index} song={song} recordVote={() => addVote(index)} />
            ))}
      </ul>
      ...
      <button onClick={toggleShowAll}>
        Show {showAll ? "all songs" : "only songs with 2 or more votes"}
      </button>
    </div>
  );
};
